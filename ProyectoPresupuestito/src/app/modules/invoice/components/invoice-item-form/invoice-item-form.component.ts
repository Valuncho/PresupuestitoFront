...
export class InvoiceItemFormComponent {
  ...
  //Properties
  currentItem: InvoceItemRequest =
    this.materialController.getEmptyInvoiceItemRequest();
  ...
  ngOnInit(): void {
    ...
    this.InvoiceSubscription = this.invoiceControllerService
      .getInvoiceId()
      .subscribe((res) => {
        this.invoiceId = res;
      });
    this.ItemSubscription = this.materialController
      .getInvoiceItem()
      .subscribe((res) => {
        this.edit = this.materialController.getEditMode();
        if (this.edit) {
    ...
    this.MaterialSubscription = this.materialController
      .getMaterial()
      .subscribe((res) => {
  ...
  ngOnDestroy(): void {
    this.materialController.setMaterial(
      this.materialController.getEmptyMaterial()
    );
  ...
  openMaterialList() {
    this.modalService.openModal<MaterialListComponent, Material>(
  ...
  setNewMaterial() {
    this.itemForm.patchValue({
      idmaterial: this.currentMaterial!.materialId,
      material: this.currentMaterial!.materialName,
    });
  }
